...
**4. Security: `/chat/` is now rate-limited** (20 requests/minute per IP — see security audit, finding #3). It remains open to anonymous (non-logged-in) visitors by design, since helping a shopper find a book before they've created an account is a reasonable thing for a storefront assistant to do — but it's no longer completely unlimited, which matters now that each message triggers local model inference (CPU/GPU work) rather than a metered third-party API call.
